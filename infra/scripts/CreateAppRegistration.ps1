...
$myApp = az app list --filter "displayName eq '$appName'" --query "[0]" -o json | ConvertFrom-Json
if ($null -eq $myApp) {
    $myApp = az app create --display-name $appName --query "{displayName: displayName, appId: appId}"  -o json | ConvertFrom-Json       
}
else {
    Write-Host "Application already exists"    
}

$credential = az app credential reset --id $myApp.appId --display-name 'MSAPP_SECRET' --append --query "password" -o tsv
