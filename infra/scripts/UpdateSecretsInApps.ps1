...
az webapp config appsettings set --name $env:AZURE_FRONTEND_WEBAPP_NAME --resource-group $env:AZURE_RESOURCE_GROUP --settings BLOB_SAS_TOKEN=$SAS_TOKEN
# Write value to Azure App Service configuration

Write-host "Generate Service Principal for Github action deployment" -ForegroundColor Gray
